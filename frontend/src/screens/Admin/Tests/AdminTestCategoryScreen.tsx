...
export function AdminTestCategoryScreen() {
  ...
  const load = () => {
    if (!period) return;
    listTests({ all: true, period }).then(({ tests }) => setTests(tests));
  };
  ...
  return (
    <div>
      <Header title={period ? uz.periods[period] : uz.admin.testManagement} showBack />
