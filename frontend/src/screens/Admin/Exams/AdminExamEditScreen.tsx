...
export function AdminExamEditScreen() {
  ...
  const handleAdd = async () => {
    if (!examId || !form.questionText.trim()) return;
    setSaving(true);
    try {
      await addExamQuestion(examId, { ...form, order: (exam?.questions?.length ?? 0) + 1 });
  ...
  const handleSaveQuestion = async (id: string, data: Partial<Question>) => {
    await updateExamQuestion(id, data);
    load();
  };
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      const result = await bulkUploadExamQuestions(examId, file);
      ...
      <div className="space-y-4 p-4">
        <div>
          <h2 className="mb-2 text-sm font-semibold text-slate-500 dark:text-slate-400">
            {uz.tests.testList} ({exam.questions?.length ?? 0})
          </h2>
